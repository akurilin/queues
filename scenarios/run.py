...
def scenario_version_order(args: argparse.Namespace, outputs: Dict) -> None:
    """Scenario: Out-of-order delivery with version-based reconciliation."""
    queue_url = outputs["queue_url"]
    region = outputs["aws_region"]
    profile = os.environ.get("AWS_PROFILE", "")
    dlq_url = queue_url_from_arn(outputs["dlq_arn"])
    sqs = build_sqs_client(region, profile)
    dynamo = build_dynamo_resource(region, profile)
    cleanup_scenario_state(sqs, dynamo, outputs, "version-order")
...
def parse_args() -> argparse.Namespace:
    ...
    vo = subparsers.add_parser(
        "version-order",
        help="Versioning scenario (out-of-order delivery with version checks)",
    )
    vo.add_argument("--count", type=int, default=10, help="Messages to send")
    vo.add_argument(
        "--entity-id",
        type=str,
        default="acct-1",
...
def main() -> None:
    ...
    scenario_fn = {
        ...
        "version-order": scenario_version_order,
